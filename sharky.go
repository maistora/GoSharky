...
func error(error []map[interface{}]interface{}) {

	// code := error[0].Code
	// msg := error[0].Message
	// log.Fatal(fmt.Sprintf("ERROR: %v: %v", code, msg))
}
...
func (sharky *Sharky) StartSession() {
	result := sharky.NoSessionCallHttp("startSession", nil)
